...
class FieldResponse:
    def __init__(self, field_type: FieldTypes, var: str, values: List[str] = None):
...
class FieldRequest:
    ...
    def __init__(
        ...
        if options is not None and field_type not in [
            FieldTypes.LIST_SINGLE.value,
            FieldTypes.LIST_MULTI.value,
        ]:
            raise ValueError(
                "Options is only available with FieldType of [LIST-SINGLE, LIST-MULTIPLE]"
            )
