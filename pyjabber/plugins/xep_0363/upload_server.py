...
class UploadHttpServer:
    ...
    def slot_request(
        self, filename: str, content_type: str = None, content_length: int = None
    ) -> Union[str, None]:
    ...
    async def handle_upload(self, request: web.Request):
        ...
        if slot_id not in self._slots.keys():
            return web.Response(
                text=f"{slot_id} not exists. Please first request an available slot via XMPP",
                status=404,
            )
        if self._slots[slot_id][SlotIndex.IS_UPLOADED_YET.value]:
            return web.Response(
                text=f"{slot_id} is already filled with a previous upload. Please request a new slot",
                status=400,
            )
        if file_name != self._slots[slot_id][SlotIndex.FILENAME]:
            return web.Response(
                text=f"{slot_id} exists, but it is not bound with the file_name: f{file_name}",
                status=404,
            )
    ...
    async def handle_download(self, request: web.Request):
        ...
        if self._slots[slot_id][SlotIndex.IS_UPLOADED_YET] is False:
            return web.Response(
                text=f"Slot {slot_id} exists, but no file were found", status=204
            )
        if file_name_req != self._slots[slot_id][SlotIndex.FILENAME]:
            return web.Response(
                text=f"{slot_id} exists, but it is not bound with the file_name: f{file_name_req}",
                status=404,
            )
