...
def setup():
    ...
    with (
        patch(
            "pyjabber.stream.StanzaHandler.ConnectionManager"
        ) as MockConnectionsManager,
...
def test_handleIQ(setup):
    ...
    child = Element(
        "{jabber:client}query"
    )  # Aseguramos que tenga el namespace adecuado
    element.append(child)

    expected_response = SE.service_unavaliable()

    with patch("pyjabber.stream.StanzaHandler.PluginManager") as MockPluginManager:
        MockPluginManager.feed.return_value = (
            expected_response  # Simulamos la respuesta esperada
        )
...
def test_handleMsg_no_resource_tie_priority(setup):
    ...
    mock_presence.return_value._most_priority.return_value = [("res1", 10), ("res2", 10)]
...
def test_handleMsg_remote_user(setup):
    ...
    mock_connections.return_value.get_server_buffer.assert_called_with(
        host="remote.com"
    )
...
def test_handleMsg_remote_user_queue(setup):
    ...
    mock_queue.put_nowait.assert_called_with(
        ("MESSAGE", "remote.com", ET.tostring(element))
    )
