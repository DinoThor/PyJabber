...
def setup_parser():
    ...
    parser = XMLServerIncomingParser(
        host, buffer, starttls, connection_manager, queue_message
    )
...
@patch(
    "pyjabber.stream.protocols.incoming.StanzaServerIncomingHandler.Presence",
    MagicMock(),
)
@patch.object(
    StreamServerIncomingHandler, "handle_open_stream", return_value=Signal.DONE
)
...
def test_end_element_ns_signal_reset(setup_parser):
    ...
    with patch.object(
        parser._streamHandler, "handle_open_stream", return_value=Signal.RESET
    ):
        parser.endElementNS(("namespace", "element"), None)
        assert not parser._stack
