...
def stream_handler(mock_buffer, mock_connection_manager):
    return StreamServerOutcomingHandler(
        "mock_host", mock_buffer, "mock_starttls", mock_connection_manager
    )
...
def test_handle_open_stream_ready(stream_handler, mock_buffer, mock_connection_manager):
    ...
    mock_connection_manager.set_server_transport.assert_called_once_with(
        peer, mock_buffer
    )
