...
import pytest
from pyjabber.network.protocols.outgoing.XMLServerOutcomingProtocol import (
...
def test_connection_made_with_transport(mock_make_parser, setup_protocol):
    ...
    connection_manager.connection_server.assert_called_with(
        "peername", protocol._host, transport
    )
...
def test_connection_made_without_transport(mock_make_parser, setup_protocol):
    ...
    with patch(
        "pyjabber.network.protocols.outgoing.XMLServerOutcomingProtocol.logger"
    ) as mock_logger:
        protocol.connection_made(transport)
