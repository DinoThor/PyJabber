...
CERTS_PATH = os.path.abspath(
    os.path.join(os.path.dirname(__file__), "../pyjabber/network/certs")
)
...
def test_check_hostname_cert_exists_all_files_present(hostname):
    with patch("os.path.isfile", return_value=True), patch("os.chdir") as mock_chdir:
        result = check_hostname_cert_exists(hostname, CERTS_PATH)
...
def test_generate_hostname_cert(hostname, file_exists):
    ...
    with (
        ...
        patch(
            "cryptography.x509.CertificateSigningRequestBuilder.sign",
            return_value=mock_csr,
        ),
        patch(
            "cryptography.hazmat.primitives.serialization.load_pem_private_key",
            return_value=mock_ca_key,
        ),
        patch("cryptography.x509.load_pem_x509_certificate", return_value=mock_ca_cert),
        patch(
            "cryptography.x509.CertificateBuilder.sign", return_value=mock_domain_cert
        ),
        patch("loguru.logger.debug") as mock_logger_debug,
