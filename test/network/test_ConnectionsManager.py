...
def test_close(connections_manager):
    ...
    mock_logger.error.assert_called_with(
        f"{('127.0.0.1', 54321)} not present in the online list"
    )
...
def test_get_buffer(connections_manager):
    ...
    connections_manager._peerList = {peer: (jid, transport, [False])}
...
def test_get_buffer_online_with_resource(connections_manager):
    ...
    transport = MagicMock(spec=Transport)
    connections_manager._peerList = {peer: (jid, transport, [True])}
...
def test_update_buffer_malformed(connections_manager):
    ...
    connections_manager.update_buffer(
        new_transport=new_transport, jid=JID("demo@localhost")
    )
    mock_logger.warning.assert_called_with(
        "JID must have a resource to update transport"
    )
    connections_manager.update_buffer(
        new_transport=new_transport, jid=JID("demo2@localhost/123")
    )
    mock_logger.warning.assert_called_with(
...
def test_set_jid(connections_manager):
    ...
    connections_manager._peerList = {peer: (None, MagicMock(), [False])}
...
def test_set_jid_key_error(connections_manager):
    ...
    log.error.assert_called_with(
        f"Unable to find {('127.0.0.1', 12345)} during jid/transport update"
    )
...
def test_update_resource(connections_manager):
    ...
    log.error.assert_called_with(
        f"Unable to find {('127.0.0.1', 54321)} during resource update"
    )
...
def test_disconnection_server(connections_manager):
    connections_manager, _ = connections_manager
    peer = ("127.0.0.1", 12345)
    connections_manager._remoteList = {peer: (None, None)}
...
def test_disconnection_server_missing_entry(connections_manager):
    ...
    assert len(connections_manager._remoteList) > 0
    mock_logger.warning.assert_called_with(
        f"Server {('127.0.0.1', 54321)} not present in the online list"
    )
...
def test_close_server(connections_manager):
    ...
    peer = ("127.0.0.1", 12345)
    transport = MagicMock()
    connections_manager._remoteList = {peer: ("remote.es", transport)}
...
def test_get_server_buffer_no_entry(connections_manager):
    ...
    mock_logger.error.assert_called_with(
        "Missing peer in the connection list. Check it"
    )
...
def test_get_server_buffer_malformed(connections_manager):
    ...
    mock_logger.error.assert_called_with(
        "Missing peer OR host to search for protocols transport. Returning None"
    )
