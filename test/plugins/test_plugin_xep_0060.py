...
def pubsub(setup_database):
    with (
        patch("pyjabber.plugins.xep_0060.xep_0060.metadata") as mock_meta,
        patch(
            "pyjabber.plugins.xep_0060.xep_0060.ConnectionManager"
        ) as mock_connection,
...
def test_update_memory_from_database(pubsub):
    ...
    assert pubsub._nodes == [
        ("TestNode", "demo", "Sample", "leaf", 1024),
        ("TestNode2", "test", "Sample", "leaf", 1024),
    ]
...
def test_feed(pubsub):
    ...
    pubsub._operations = {"create": mock_operation}
...
def test_feed_exception(pubsub):
    ...
    jid = JID("test@localhost")
    with (
        patch("pyjabber.plugins.xep_0060.xep_0060.StanzaError") as mock_se,
...
def test_create_node(pubsub):
    ...
    assert (
        res
        == b'<iq xmlns:ns0="http://jabber.org/protocol/pubsub" id="items1" from="localhost" type="result"><ns0:pubsub><create node="TestNode3" /></ns0:pubsub></iq>'
    )
...
def test_create_node_no_node(pubsub):
    ...
    assert (
        res
        == b'<iq xmlns:ns0="urn:ietf:params:xml:ns:xmpp-stanzas" xmlns:ns1="http://jabber.org/protocol/pubsub#errors" id="items1" to="test@localhost" type="error"><error type="auth"><ns0:not-acceptable /><ns1:nodeid-required /></error></iq>'
    )
...
def test_create_node_conflict(pubsub):
    ...
    assert (
        res
        == b'<iq xmlns:ns0="urn:ietf:params:xml:ns:xmpp-stanzas" id="items1" to="test@localhost" type="error"><error type="auth"><ns0:conflict /></error></iq>'
    )
...
def test_delete_node(pubsub):
    ...
    assert (
        res
        == b'<iq xmlns:ns0="http://jabber.org/protocol/pubsub" id="items1" from="localhost" type="result"><ns0:pubsub /></iq>'
    )
...
def test_delete_node_no_node(pubsub):
    ...
    assert (
        res
        == b'<iq xmlns:ns0="urn:ietf:params:xml:ns:xmpp-stanzas" xmlns:ns1="http://jabber.org/protocol/pubsub#errors" id="items1" to="demo@localhost" type="error"><error type="auth"><ns0:not-acceptable /><ns1:nodeid-required /></error></iq>'
    )
...
def test_delete_node_not_found(pubsub):
    ...
    assert (
        res
        == b'<iq xmlns:ns0="urn:ietf:params:xml:ns:xmpp-stanzas" id="items1" to="demo@localhost" type="error"><error type="cancel"><ns0:item-not-found /></error></iq>'
    )
...
def test_delete_node_forbidden(pubsub):
    ...
    assert (
        res
        == b'<iq xmlns:ns0="urn:ietf:params:xml:ns:xmpp-stanzas" id="items1" to="test@localhost" type="error"><error type="auth"><ns0:forbidden /></error></iq>'
    )
...
def test_retrieve_items_node_forbidden(pubsub):
    ...
    assert (
        res
        == b'<iq xmlns:ns0="urn:ietf:params:xml:ns:xmpp-stanzas" id="items1" to="fake@localhost" type="error"><error type="auth"><ns0:forbidden /></error></iq>'
    )
...
def test_retrieve_items_node_not_found(pubsub):
    ...
    assert (
        res
        == b'<iq xmlns:ns0="urn:ietf:params:xml:ns:xmpp-stanzas" id="items1" to="fake@localhost" type="error"><error type="cancel"><ns0:item-not-found /></error></iq>'
    )
...
def test_retrieve_items_node_no_node(pubsub):
    ...
    assert (
        res
        == b'<iq xmlns:ns0="urn:ietf:params:xml:ns:xmpp-stanzas" xmlns:ns1="http://jabber.org/protocol/pubsub#errors" id="items1" to="fake@localhost" type="error"><error type="auth"><ns0:not-acceptable /><ns1:nodeid-required /></error></iq>'
    )
...
def test_subscribe(pubsub):
    ...
    with patch("pyjabber.plugins.xep_0060.xep_0060.uuid4") as mock_uuid:
        ...
        assert any(
            s == ("TestNode", "fake", "987654321", "subscribed", "publisher")
            for s in pubsub._subscribers
        )
        ...
        assert all(
            prop in res_query
            for prop in ["TestNode", "fake", "987654321", "subscribed", "publisher"]
        )
        assert (
            res
            == b'<iq xmlns:ns0="http://jabber.org/protocol/pubsub" id="sub" from="localhost" type="result"><ns0:pubsub><subscription node="TestNode" jid="fake@localhost" subid="987654321" subscription="subscribed" /></ns0:pubsub></iq>'
        )
...
def test_subscribe_no_node(pubsub):
    ...
    assert (
        res
        == b'<iq xmlns:ns0="urn:ietf:params:xml:ns:xmpp-stanzas" xmlns:ns1="http://jabber.org/protocol/pubsub#errors" id="sub" to="fake@localhost" type="error"><error type="auth"><ns0:not-acceptable /><ns1:nodeid-required /></error></iq>'
    )
...
def test_subscribe_no_jid(pubsub):
    ...
    assert (
        res
        == b'<iq xmlns:ns0="urn:ietf:params:xml:ns:xmpp-stanzas" xmlns:ns1="http://jabber.org/protocol/pubsub#errors" id="sub" to="fake@localhost" type="error"><error type="modify"><ns0:bad-request /><ns1:invalid-jid /></error></iq>'
    )
...
def test_subscribe_not_found(pubsub):
    ...
    assert (
        res
        == b'<iq xmlns:ns0="urn:ietf:params:xml:ns:xmpp-stanzas" id="sub" to="fake@localhost" type="error"><error type="cancel"><ns0:item-not-found /></error></iq>'
    )
...
def test_subscribe_subscribed_already(pubsub):
    ...
    assert (
        res
        == b'<iq xmlns:ns0="http://jabber.org/protocol/pubsub" id="sub" from="localhost" type="result"><ns0:pubsub><subscription node="TestNode" jid="test@localhost" subid="123456789" subscription="subscribed" /></ns0:pubsub></iq>'
    )
...
def test_subscribe_subscribed_pending(pubsub):
    ...
    assert (
        res
        == b'<iq xmlns:ns0="urn:ietf:params:xml:ns:xmpp-stanzas" xmlns:ns1="http://jabber.org/protocol/pubsub#errors" id="sub" to="dump@localhost" type="error"><error type="auth"><ns0:not-authorized /><ns1:pending-subscription /></error></iq>'
    )
...
def test_unsubscribe(pubsub):
    ...
    assert (
        res
        == b'<iq xmlns:ns0="http://jabber.org/protocol/pubsub" id="sub" from="localhost" type="result"><ns0:pubsub><subscription node="TestNode" jid="test@localhost" subscription="none" /></ns0:pubsub></iq>'
    )
...
def test_unsubscribe_no_node(pubsub):
    ...
    assert (
        res
        == b'<iq xmlns:ns0="urn:ietf:params:xml:ns:xmpp-stanzas" xmlns:ns1="http://jabber.org/protocol/pubsub#errors" id="sub" to="test@localhost" type="error"><error type="auth"><ns0:not-acceptable /><ns1:nodeid-required /></error></iq>'
    )
...
def test_unsubscribe_no_jid(pubsub):
    ...
    assert (
        res
        == b'<iq xmlns:ns0="urn:ietf:params:xml:ns:xmpp-stanzas" xmlns:ns1="http://jabber.org/protocol/pubsub#errors" id="sub" to="test@localhost" type="error"><error type="modify"><ns0:bad-request /><ns1:invalid-jid /></error></iq>'
    )
...
def test_unsubscribe_invalid_jid(pubsub):
    ...
    assert (
        res
        == b'<iq xmlns:ns0="urn:ietf:params:xml:ns:xmpp-stanzas" xmlns:ns1="http://jabber.org/protocol/pubsub#errors" id="sub" to="tset@localhost" type="error"><error type="modify"><ns0:bad-request /><ns1:invalid-jid /></error></iq>'
    )
...
def test_unsubscribe_item_not_found(pubsub):
    ...
    assert (
        res
        == b'<iq xmlns:ns0="urn:ietf:params:xml:ns:xmpp-stanzas" id="sub" to="test@localhost" type="error"><error type="cancel"><ns0:item-not-found /></error></iq>'
    )
...
def test_unsubscribe_not_subscribed(pubsub):
    ...
    assert (
        res
        == b'<iq xmlns:ns0="urn:ietf:params:xml:ns:xmpp-stanzas" xmlns:ns1="http://jabber.org/protocol/pubsub#errors" id="sub" to="dummy@localhost" type="error"><error type="cancel"><ns0:unexpected-request /><ns1:not-subscribed /></error></iq>'
    )
...
def test_unsubscribe_need_subid(pubsub):
    ...
    assert (
        res
        == b'<iq xmlns:ns0="urn:ietf:params:xml:ns:xmpp-stanzas" xmlns:ns1="http://jabber.org/protocol/pubsub#errors" id="sub" to="unsub@localhost" type="error"><error type="modify"><ns0:bad-request /><ns1:subid-required /></error></iq>'
    )
...
def test_unsubscribe_invalid_subid(pubsub):
    ...
    assert (
        res
        == b'<iq xmlns:ns0="urn:ietf:params:xml:ns:xmpp-stanzas" xmlns:ns1="http://jabber.org/protocol/pubsub#errors" id="sub" to="unsub@localhost" type="error"><error type="modify"><ns0:not-acceptable /><ns1:invalid-subid /></error></iq>'
    )
...
def test_retrieve_subscriptions(pubsub):
    ...
    assert (
        res
        == b'<iq xmlns:ns0="http://jabber.org/protocol/pubsub" id="sub" from="localhost" type="result"><ns0:pubsub><ns0:subscriptions><ns0:subscription node="TestNode" jid="test@localhost" subscription="subscribed" subid="123456789" /></ns0:subscriptions></ns0:pubsub></iq>'
    )
...
def test_retrieve_subscriptions_forbidden(pubsub):
    ...
    assert (
        res
        == b'<iq xmlns:ns0="urn:ietf:params:xml:ns:xmpp-stanzas" id="sub" to="test@localhost" type="error"><error type="auth"><ns0:forbidden /></error></iq>'
    )
...
def test_purge(pubsub):
    ...
    assert (
        res
        == b'<iq xmlns:ns0="http://jabber.org/protocol/pubsub#owner" id="sub" from="localhost" type="result"><ns0:pubsub /></iq>'
    )
...
def test_purge_not_found(pubsub):
    ...
    assert (
        res
        == b'<iq xmlns:ns0="urn:ietf:params:xml:ns:xmpp-stanzas" id="sub" to="demo@localhost" type="error"><error type="cancel"><ns0:item-not-found /></error></iq>'
    )
...
def test_retract(pubsub):
    ...
    query = select(Model.PubsubItems).where(
        and_(
            Model.PubsubItems.c.item_id == "123", Model.PubsubItems.c.node == "TestNode"
        )
    )
    with engine.connect() as con:
...
def test_retract_no_node(pubsub):
    ...
    assert (
        res
        == b'<iq xmlns:ns0="urn:ietf:params:xml:ns:xmpp-stanzas" xmlns:ns1="http://jabber.org/protocol/pubsub#errors" id="sub" to="demo@localhost" type="error"><error type="modify"><ns0:bad-request /><ns1:nodeid-required /></error></iq>'
    )
...
def test_retract_no_item(pubsub):
    ...
    assert (
        res
        == b'<iq xmlns:ns0="urn:ietf:params:xml:ns:xmpp-stanzas" xmlns:ns1="http://jabber.org/protocol/pubsub#errors" id="sub" to="demo@localhost" type="error"><error type="modify"><ns0:bad-request /><ns1:item-required /></error></iq>'
    )
...
def test_retract_forbidden(pubsub):
    ...
    assert (
        res
        == b'<iq xmlns:ns0="urn:ietf:params:xml:ns:xmpp-stanzas" id="sub" to="lac@localhost" type="error"><error type="auth"><ns0:forbidden /></error></iq>'
    )
