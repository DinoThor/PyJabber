...
def setup(setup_database):
    with (
        patch("pyjabber.plugins.roster.Roster.DB") as mock_db,
...
def test_handleGet_existing_jid(setup):
    ...
    assert (
        b'<iq id="1234" type="result">' in result
    )  # Verificamos que el resultado contiene la respuesta IQ correcta
...
def test_handleGet_non_existing_jid(setup):
    ...
    assert (
        b'<iq id="1234" type="result">' in result
    )  # Verificamos que se maneja adecuadamente aunque el jid no exista
...
def test_handleSet_invalid_xml(setup):
    ...
    query = ET.SubElement(element, "{jabber:iq:roster}query")
    ET.SubElement(
        query, "{jabber:iq:roster}item", attrib={"jid": "jid1", "subscription": "both"}
    )
    ET.SubElement(
        query, "{jabber:iq:roster}item", attrib={"jid": "jid2", "subscription": "none"}
    )
