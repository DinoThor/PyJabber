...
def setup_presence(setup_database, model):
    with (
        patch("pyjabber.features.presence.PresenceFeature.metadata") as mock_meta,
        patch(
            "pyjabber.features.presence.PresenceFeature.ConnectionManager"
...
def test_handle_subscribe(setup_presence):
    ...
    element = ET.Element(
        "presence", attrib={"type": "subscribe", "to": "user@localhost", "id": "123"}
    )
    mock_roster.roster_by_jid.side_effect = [
        [
            {"id": 1, "item": '<item jid="user@localhost" subscription="none"/>'}
        ],  # Initial simulation
        [
            {
                "id": 1,
                "item": '<item jid="user@localhost" subscription="none" ask="subscribe"/>',
            }
        ],  # After update
    ]
    ...
    expected_item = ET.Element(
        "item",
        attrib={"jid": "user@localhost", "subscription": "none", "ask": "subscribe"},
    )
...
def test_handle_subscribe_non_localhost(setup_presence):
    presence, mock_connections, mock_roster = setup_presence
    element = ET.Element(
        "presence", attrib={"type": "subscribe", "to": "user@domain.com", "id": "123"}
    )
    jid = JID("user2@localhost")
    result = presence.handle_subscribe(jid, element)
...
def test_handle_global_presence_no_roster_entries(setup_presence):
    ...
    assert presence._online_status == {
        jid.bare(): [(jid.resource, PresenceType.AVAILABLE, None, None, None)]
    }
...
def test_feed_handle_subscribed(setup_presence):
    presence, mock_connections, mock_roster = setup_presence
    element = ET.Element(
        "presence", attrib={"type": "subscribed", "to": "user@localhost", "id": "123"}
    )
    jid = JID("user2@localhost/1223")
    mock_roster.roster_by_jid.side_effect = [
        [
            {"id": 1, "item": '<item jid="user2" subscription="none" ask="subscribe"/>'}
        ],  # Initial simulation
    ...
    expected_fist_update = ET.fromstring(
        '<item jid="user2@localhost" subscription="to"/>'
    )
    expected_second_update = ET.fromstring(
        '<item jid="user@localhost" subscription="from"/>'
    )
...
def test_feed_handle_unsubscribed(setup_presence):
    presence, mock_connections, mock_roster = setup_presence
    element = ET.Element(
        "presence", attrib={"type": "unsubscribed", "to": "user@localhost", "id": "123"}
    )
    jid = JID("user2@localhost/1223")
    mock_roster.roster_by_jid.side_effect = [
    ...
    mock_connections.get_buffer.return_value = [
        (JID("user@localhost/123"), MagicMock())
    ]
    presence._roster = mock_roster
    presence._connections = mock_connections
...
def test_feed_handle_unavailable(setup_presence):
    ...
    element = ET.Element(
        "presence",
        attrib={"type": "unavailable", "from": "user2@localhost", "id": "123"},
    )
    jid = JID("user2@localhost")
    result = presence.feed(jid, element)
    assert result is None
...
def test_handle_subscribed(setup_presence):
    ...
    mock_connections.get_buffer.side_effect = [[MagicMock()], [MagicMock()]]
